...
# Import quote-based strategies (if available)
try:
    from quote_based_strategies import (
        OFIMicroMomentumStrategy,
        TopBookImbalanceStrategy,
        SpreadCompressionBreakoutStrategy,
        QueueDynamicsStrategy,
        LiquidityHoleStrategy
    )
    QUOTE_STRATEGIES_AVAILABLE = True
except ImportError:
    # Quote strategies not available (quote_based_strategies.py not found or dependencies missing)
    QUOTE_STRATEGIES_AVAILABLE = False
    OFIMicroMomentumStrategy = None
    TopBookImbalanceStrategy = None
    SpreadCompressionBreakoutStrategy = None
    QueueDynamicsStrategy = None
    LiquidityHoleStrategy = None
...
# Add quote-based strategies to registry if available
if QUOTE_STRATEGIES_AVAILABLE:
    STRATEGY_REGISTRY.update({
        'OFIMicroMomentumStrategy': OFIMicroMomentumStrategy,
        'TopBookImbalanceStrategy': TopBookImbalanceStrategy,
        'SpreadCompressionBreakoutStrategy': SpreadCompressionBreakoutStrategy,
        'QueueDynamicsStrategy': QueueDynamicsStrategy,
        'LiquidityHoleStrategy': LiquidityHoleStrategy
    })


def create_strategy(strategy_name: str, timeframe: str, params: Optional[Dict] = None) -> BaseStrategy:
    """
    Factory function to create a strategy instance.
    
    Args:
        strategy_name: Name of the strategy (e.g., 'MLTrendStrategy')
        timeframe: Trading timeframe (e.g., '5T', '15T', '30T')
        params: Optional parameters dictionary to override defaults
    ...
    if strategy_name not in STRATEGY_REGISTRY:
        # Try to import quote strategies lazily if not already loaded
        if strategy_name in [
            'OFIMicroMomentumStrategy', 'TopBookImbalanceStrategy',
            'SpreadCompressionBreakoutStrategy', 'QueueDynamicsStrategy',
            'LiquidityHoleStrategy'
        ]:
            try:
                from quote_based_strategies import (
                ...
                STRATEGY_REGISTRY.update(quote_strategies)
                # Try again now that they're registered
