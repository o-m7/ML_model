#!/usr/bin/env python3
"""
FIX GOLD USING THE WORKING SYSTEM
==================================

Use EXACT SAME code as production_final_system.py that got 7 models working.
Just adjust XAUUSD parameters to match what works.
"""
...
class Config:
    ...
    # Use SAME parameters that worked for other pairs
    # XAGUSD 15T worked with: tp=1.5, conf=0.38, pos=0.30
    # AUDUSD 15T worked with: tp=1.5, conf=0.38, pos=0.70
    # NZDUSD 15T worked with: tp=1.5, conf=0.38, pos=0.60
    
    XAUUSD_PARAMS = {
        '15T': {
            'tp': 1.5,           # SAME as working models
            'sl': 1.0,
            'min_conf': 0.38,    # SAME as working models
            'min_edge': 0.10,    # SAME as working models
            'pos_size': 0.50,    # Conservative (between XAGUSD 0.3 and AUDUSD 0.7)
        },
        '1H': {
            'tp': 1.6,           # Slightly higher for 1H
            'sl': 1.0,
            'min_conf': 0.38,    # SAME as working models
            'min_edge': 0.10,    # SAME as working models
            'pos_size': 0.60,    # Moderate
        },
    }
...
def create_balanced_labels(df: pd.DataFrame, symbol: str, timeframe: str, 
    ...
    """EXACT SAME labeling as working system."""
...
def add_features(df: pd.DataFrame) -> pd.DataFrame:
    """EXACT SAME features as working system."""
    df = df.copy()
...
def select_features(df: pd.DataFrame) -> list:
    """EXACT SAME feature selection as working system."""
...
class BalancedModel:
    """EXACT SAME model as working system."""
...
class ProductionBacktest:
    """EXACT SAME backtest as working system."""
...
def fix_gold(timeframe: str):
    """Fix Gold using EXACT SAME approach that got 7 models working."""
    
    print(f"\n{'='*80}")
    print(f"FIX GOLD: XAUUSD {timeframe}")
    print(f"Using EXACT SAME system that got 7 models working")
    print(f"{'='*80}\n")
    
    try:
        params = CONFIG.XAUUSD_PARAMS.get(timeframe)
        
        print(f"Parameters (SAME as working models):")
        print(f"  TP={params['tp']}, Conf={params['min_conf']}, Pos={params['pos_size']*100:.0f}%\n")
        ...
        df = add_features(df)
        df = create_balanced_labels(df, 'XAUUSD', timeframe, params['tp'], params['sl'])
        ...
        engine = ProductionBacktest(test_df, 'XAUUSD', params['pos_size'])
        ...
        print(f"\n{'✅ FIXED!' if passed else '❌ ' + ', '.join(failures)}")
        print(f"{'='*80}\n")
...
def main():
    ...
    print(f"\nFINAL: {passed}/{len(results)} FIXED\n")
